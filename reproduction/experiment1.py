...
for _i_sim in range(N_SIMULATION):
    ...
    r_index_Z = np.random.choice(
      len(Z_test_mat), size=len(Z_test_mat), replace=False)
    Z_test_intervention = Z_test_mat[r_index_Z, :]
    Z1_intervention = Z_test_intervention[:, [0]]
    Z2_intervention = Z_test_intervention[:, [1]]
    Z3_intervention = Z_test_intervention[:, [2]]
    Z4_intervention = Z_test_intervention[:, [3]]
    Z5_intervention = Z_test_intervention[:, 4:]
